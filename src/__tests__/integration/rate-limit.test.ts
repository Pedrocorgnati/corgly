...
describe('checkRateLimit — unit (AUTH_006, RATE_001)', () => {
  ...
  it('permite as primeiras N requisições dentro da janela', async () => {
    const key = `test-allow-${Date.now()}`;
    for (let i = 0; i < RATE_LIMITS.AUTH_LOGIN.maxRequests; i++) {
      const result = await checkRateLimit(key, RATE_LIMITS.AUTH_LOGIN);
      expect(result.allowed).toBe(true);
    }
  });

  it('bloqueia a N+1ª requisição — AUTH_006', async () => {
    const key = `test-block-${Date.now()}`;
    // Consume all allowed slots
    for (let i = 0; i < RATE_LIMITS.AUTH_LOGIN.maxRequests; i++) {
      await checkRateLimit(key, RATE_LIMITS.AUTH_LOGIN);
    }
  ...
  it('reseta o contador após a janela de tempo expirar', async () => {
    ...
    // Blocked
    expect((await checkRateLimit(key, RATE_LIMITS.AUTH_LOGIN)).allowed).toBe(false);
  ...
  it('bloqueia forgot-password na 4ª tentativa em 15min — AUTH_006', async () => {
    const key = `forgot-${Date.now()}`;
    for (let i = 0; i < RATE_LIMITS.AUTH_FORGOT.maxRequests; i++) {
      const r = await checkRateLimit(key, RATE_LIMITS.AUTH_FORGOT);
      expect(r.allowed).toBe(true);
    }
    const blocked = await checkRateLimit(key, RATE_LIMITS.AUTH_FORGOT);
    expect(blocked.allowed).toBe(false);
  });

  it('bloqueia rota geral após 101 requisições — RATE_001', async () => {
    const key = `general-${Date.now()}`;
    for (let i = 0; i < RATE_LIMITS.GENERAL.maxRequests; i++) {
      await checkRateLimit(key, RATE_LIMITS.GENERAL);
    }
    const blocked = await checkRateLimit(key, RATE_LIMITS.GENERAL);
    expect(blocked.allowed).toBe(false);
  ...
  it('chaves diferentes não interferem entre si', async () => {
    ...
    for (let i = 0; i < RATE_LIMITS.AUTH_LOGIN.maxRequests; i++) {
      await checkRateLimit(key1, RATE_LIMITS.AUTH_LOGIN);
    }
    expect((await checkRateLimit(key1, RATE_LIMITS.AUTH_LOGIN)).allowed).toBe(false);

    // key2 is unaffected
    const key2Result = await checkRateLimit(key2, RATE_LIMITS.AUTH_LOGIN);
